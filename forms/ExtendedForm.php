	...
	protected function compile()
	{
		...
		if (isset($GLOBALS['TL_HOOKS']['compileFormFields']) && is_array($GLOBALS['TL_HOOKS']['compileFormFields']))
		{
			foreach ($GLOBALS['TL_HOOKS']['compileFormFields'] as $callback)
			{
				...
				$arrFields = $this->{$callback[0]}->{$callback[1]}($arrFields, $formId, $this); //190314CM: PHP7.1-Fix
			}
		}
		// Process the fields
				...
				if (!strlen($_POST['FORM_BACK']) && !strlen($_POST['FORM_BACK_x']))
				{
					if (isset($GLOBALS['TL_HOOKS']['loadFormField']) && is_array($GLOBALS['TL_HOOKS']['loadFormField']))
					{
						foreach ($GLOBALS['TL_HOOKS']['loadFormField'] as $callback)
						{
                            ...
                            $objWidget = $this->{$callback[0]}->{$callback[1]}($objWidget, $formId, $this->arrData, $form); //190314CM: PHP7.1-Fix mit Rocksolid-Antispam
				...
				if (\Input::post('FORM_SUBMIT') == $formId)
				{
					...
					if (!$doNotValidate)
					{
						...
						if (isset($GLOBALS['TL_HOOKS']['validateFormField']) && is_array($GLOBALS['TL_HOOKS']['validateFormField']))
						{
							...
							{
								$this->import($callback[0]);
								$objWidget = $this->{$callback[0]}->{$callback[1]}($objWidget, $formId, $this->arrData); //190314CM: PHP7.1-Fix
							}
						}
					}
...
<script>' . $this->getBackButtonJavascriptString() . '
		...
		if ($blnAddDateJS && $objPage->hasMooTools) // 190314CM: Mootools-Fehler https://community.contao.org/de/showthread.php?48857-Mehrseitiges-Formular-Javascript-Fehler-bei-quot-Locale-is-not-defined-quot
		{
			$this->Template->fields .= '
