...
class FormdataProcessor extends \Frontend
{
	...
	public function processSubmittedData($arrSubmitted, $arrForm=false, $arrFiles=false, $arrLabels=false) {
			...
			*/
            ...
            if (array_key_exists('processEfgFormData', $GLOBALS['TL_HOOKS']) && is_array($GLOBALS['TL_HOOKS']['processEfgFormData']))
            {
                foreach ($GLOBALS['TL_HOOKS']['processEfgFormData'] as $key => $callback)
                {
                    ...
                    // # Weg 1: Fixen über call_user_func_array() // 200116^lb + ^ff
                    #$arrResult = call_user_func_array([$callback[0], $callback[1]], [$arrToSave, $arrFiles, $intOldId, $arrForm, $arrLabels]);

                    // # Weg 2: static::importStatic() + Curly braces syntax // 200116^lb + ^ff
                    $arrResult = static::importStatic($callback[0])->{$callback[1]}($arrToSave, $arrFiles, $intOldId, $arrForm, $arrLabels);
		...
		//exit;
        //var_export($objWidget,1)
        //file_put_contents("/kunden/226796_91058/kunden/agentur-cms.com/efg_MailObjects.log",var_export($this->Formdata,1),FILE_APPEND);
		...
		if ($arrForm['sendFormattedMail'])
		{
			...
			if (!empty($arrRecipient))
			{
				foreach ($arrRecipient as $kR => $recipient)
				{
                    //181122CM: Contao kennt diesen InsertTag nicht und würde ihn hier rausschmeißen. vgl. Formdata.php, 181122CM Schritt 1-5
					//list($recipientName, $recipient) = \StringUtil::splitFriendlyEmail($this->replaceInsertTags($recipient, false));
                    list($recipientName, $recipient) = \StringUtil::splitFriendlyEmail($recipient);
					$arrRecipient[$kR] = (strlen($recipientName) ? $recipientName . ' <' . $recipient . '>' : $recipient);
				}
			}
			$objMailProperties->recipients = $arrRecipient;
			...
			if (!empty($objMailProperties->recipients))
			{
				...
				foreach ($objMailProperties->recipients as $recipient)
				{
				    if($arrForm['lbCert'] !== NULL)
				    {
                        $objMail->setPublicCertPath(TL_ROOT . '/' . \FilesModel::findByUuid($arrForm['lbCert'])->path);
                        $objMail->sendSMimeTo($recipient);
                    }
				    else
				    {
                        $objMail->sendTo($recipient);
