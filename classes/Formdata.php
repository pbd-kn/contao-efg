...
class Formdata extends \Frontend
{
	...
	public function prepareMailData($objMailProperties, $arrSubmitted, $arrFiles, $arrForm, $arrFormFields)
	{
		...
		//181122CM: Schritt 1: Hin-Konvertieren das gleiche für Recipient
        if (!empty($arrRecipients) && is_array($arrRecipients))
        {
            for ($i=0;$i<count($arrRecipients);$i++)
            {
                $arrRecipients[$i] = preg_replace(array('/\{\{/', '/\}\}/'), array('__BRCL__', '__BRCR__'), $arrRecipients[$i]);
            }
        }
        ...
        //181122CM: Schritt 2: EFG-Ersetzen. Nicht nötig, da wir die if-Conditions nicht berücksichtigen
        //$blnEvalRecipient = $this->replaceConditionTags($arrRecipients);
        ...
        //181122CM: Schritt 3: auch die Recipients überprüfen und durchgehen
        preg_match_all('/__BRCL__.*?__BRCR__/si', $messageText . $messageHtml . $subject . $sender . $senderName . implode("",$arrRecipients), $tags);
		...
		foreach ($tags[0] as $tag)
		{
			...
			switch (strtolower($elements[0]))
			{
				...
				case 'form':
                    ...
                    // 181122CM: Schritt 4: Gehe durch Recipients und ersetze die InsertTags
                    if (!empty($arrRecipients))
                    {
                        for ($i = 0; $i < count($arrRecipients); $i++)
                        {
                            $arrRecipients[$i] = str_replace($tag, $strVal, $arrRecipients[$i]);
                        }
                    }
					break;
			}
        ...
        // 181122CM: Schritt 5: Gehe durch Recipients und aktiviere wieder die normale InsertTag-Routine
        if (!empty($arrRecipients) && is_array($arrRecipients))
        {
            for ($i = 0; $i < count($arrRecipients); $i++)
            {
                $arrRecipients[$i] = preg_replace(array('/__BRCL__/', '/__BRCR__/'), array('{{', '}}'), $arrRecipients[$i]);
                $arrRecipients[$i] = trim($this->replaceInsertTags($arrRecipients[$i], false));
            }
        }
